...
public class MaidSlashBladeMove {
    ...
    private static boolean handleMove(
        ...
        if (mob instanceof EntityMaid maid) {
            ...
            if (MaidGuardHandler.isGuarding(maid) && !hasNativePower) {
                return false;
            }
            ...
            if (targetVisible && distance > reach * reach) {
                ...
                walkTargetAccessor.set(new WalkTarget(new EntityTracker(target, false), speedModifier.apply(mob) * (hasNativePower ? 2 : 1), 0));
            }
